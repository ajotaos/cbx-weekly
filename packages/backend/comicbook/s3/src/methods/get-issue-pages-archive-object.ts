...
import { issuePagesArchiveBucketObjectMetadataSchema } from '../types';
...
export async function getIssuePagesArchiveObjectFromBucket(
	props: GetIssuePagesArchiveObjectFromBucket.Props,
	bucketName: string,
	client: S3Client,
) {
	...
	return client
		...
		.then(({ Body, Metadata }) => {
			if (Body === undefined) {
				throwObjectNotFoundError('issue_pages_archive', { id, issueId });
			}
...
const parseMetadata = v.parser(issuePagesArchiveBucketObjectMetadataSchema);

export declare namespace GetIssuePagesArchiveObjectFromBucket {
	type Props = {
		id: string;
		issueId: string;
	};
